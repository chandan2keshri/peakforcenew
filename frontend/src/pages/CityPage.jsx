...
//   // Sort localities with selected ones first
//   const sortedLocalities = useMemo(() => {
//     const selected = selectedFilters["Localities"] || [];
//     return [...localities].sort((a, b) => {
//       const aSelected = selected.includes(a);
//       const bSelected = selected.includes(b);
//       return bSelected - aSelected;
//     });
//   }, [localities, selectedFilters]);

//   const filteredLocalities = sortedLocalities.filter((locality) =>
...
//   return (
//     <div className={`mt-4 border-b border-gray-200 pb-3 ${className}`}>
//       {showTitle && (
...
//     // Check if last segment is a valid city in our localities
...
//     if (
//       matchedLocality &&
//       !selectedFilters["Localities"]?.includes(matchedLocality)
//     ) {
//       handleFilterChange("Localities", matchedLocality);
//     }
//   }, [location.pathname]);
...
//       // For localities, replace existing values to avoid duplicates
//       if (category === "Localities") {
//         newFilters[category] = [option]; // Replace with new locality
//       } else {
//         // For other categories, toggle the option
...
//       {/* Project Type Toggle */}
//       <div className="w-full text-center mb-0 lg:mb-8 mt-10 bg-gradient-to-r from-blue-50 to-purple-50 py-0">
  ...
  return (
    <div className={`mt-4 border-b border-gray-200 pb-3 ${className}`}>
      {showTitle && (
        <div className="flex justify-between items-center text-md font-semibold text-gray-800">
          Localities
          {selectedFilters["Localities"]?.length > 0 && (
            <span className="text-sm font-normal text-gray-500">
              {selectedFilters["Localities"].length} selected
            </span>
          )}
        </div>
      )}
...
function FiltersWithProjects() {
  ...
  const [areaUnit, setAreaUnit] = useState("sqft");
  const [projectType, setProjectType] = useState("residential");
  const [initialUrlLocalityLoaded, setInitialUrlLocalityLoaded] = useState(false);

  // Extract city from URL on first load
  useEffect(() => {
    if (!initialUrlLocalityLoaded) {
      const pathSegments = location.pathname.split('/').filter(Boolean);
      const lastSegment = pathSegments[pathSegments.length - 1];
      
      // Check if last segment is a valid city in our localities
      const matchedLocality = localities.find(locality => 
        locality.toLowerCase().replace(/\s+/g, '-') === lastSegment.toLowerCase()
      );
      
      if (matchedLocality) {
        setSelectedFilters(prev => ({
          ...prev,
          Localities: [...(prev.Localities || []), matchedLocality]
        }));
      }
      setInitialUrlLocalityLoaded(true);
    }
  }, [location.pathname, initialUrlLocalityLoaded]);
